...
class Bezier(Module):
    ...
    def forward(self, t):
        binom_coeffs = self.binom * torch.pow(t, self.range) * torch.pow((1.0 - t), self.rev_range)
        return binom_coeffs
...
class PolyChain(Module):
    ...
    def forward(self, t):
        ...
        poly_coeffs = torch.max(self.range.new([0.0]), 1.0 - torch.abs(t_n - self.range))
        return poly_coeffs
...
class Conv3d(CurveModule):
    def __init__(self, in_channels, out_channels, kernel_size, fix_points, stride=1,
                 padding=0, dilation=1, groups=1, bias=True):
        super(Conv3d, self).__init__(fix_points, ('weight', 'bias'))
        if in_channels % groups != 0:
            raise ValueError('in_channels must be divisible by groups')
        if out_channels % groups != 0:
            raise ValueError('out_channels must be divisible by groups')
    ...
    def forward(self, input, coeffs_t):
        weight_t, bias_t = self.compute_weights_t(coeffs_t)
        return F.conv3d(input, weight_t, bias_t, self.stride,
                        self.padding, self.dilation, self.groups)
...
class CurveNet(Module):
    def __init__(self, num_classes, device, curve, architecture, num_bends, fix_start=True, fix_end=True,
        ...
        self.net = self.architecture(num_classes, device, fix_points=self.fix_points, **architecture_kwargs)
        self.curve_modules = []
