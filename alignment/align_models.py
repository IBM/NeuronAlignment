...
# Warm-start allows for quicker alignment, but is more memory intensive.
...
# Align the second model to the first model
if args.model == 'TinyTen' or args.model == 'VGG16' or args.model == 'ConvFC':
    _, _, corr_unaligned_mean, _ = \
        alignment.compute_model_alignment(model[0], model[1], loaders['test_align'])
    print('Unaligned Mean Correlation', corr_unaligned_mean)  
    hard_match, _, _, corr_aligned_train = \
        alignment.compute_model_alignment(model[0], model[1], loaders['align'],
                                          quad_assignment=False)
    ...
    np.save('%s%s_aligned_mean_seeds_%02d_%02d.npy' % (args.dir2, args.align_name, args.seed_a, args.seed_b),
            corr_aligned_mean)
# A different method is called to handle the residual blocks in ResNet architectures
